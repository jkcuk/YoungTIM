...
implements Serializable
{
	...
	public void makeBessel(double topologicalCharge, double radialWaveNumber, double xCentre, double yCentre)
	{
		BesselJ besselJ = new BesselJ(topologicalCharge);
		
		// BesselJ.value(topologicalCharge, radialWaveNumber*)
		
		for(int i=0; i<width; i++)
		{
			double x = getX(i) - xCentre;	// x
			
			for(int j=0; j<height; j++)
			{
				double y = getY(j) - yCentre;	// y
				double r = Math.sqrt(x*x + y*y); 	// r

				double phase = topologicalCharge * Math.atan2(y, x);
				setElement(i, j, 
						Complex.product(
								besselJ.value(radialWaveNumber*r),	// amplitude
								new Complex(Math.cos(phase), Math.sin(phase)))	// phase term
				);
			}
		}
	}
	
	public void makeUniformPlaneWave(double xWaveNumber, double yWaveNumber)
	{
		for(int i=0; i<width; i++)
		{
			double x = getX(i);	// x
			...
			for(int j=0; j<height; j++)
			{
				...
				setElement(i, j, Complex.expI(xWaveNumber*x + yWaveNumber*y));
			}
		}
	}
